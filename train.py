   ...
   # epochのループ
    for epoch in range(num_epochs):
        ...
        for imges in dataloader:
            ...
            label_real = torch.full((mini_batch_size,), 1, dtype=float).to(device)
            label_fake = torch.full((mini_batch_size,), 0, dtype=float).to(device)
...
def validate(G_update: nn.Module):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
